        ...
        switch (tag) {
            case 'id':
                if (state.controller.editingId !== value) {
                    ...
                    // 设置当前项正在被编辑
                    // 禁止重复设置当前编辑项
                    if (state.activationItem.moduleId === value) return;
                    
                    for (let index = 0; index < state.appData.length; index++) {
                        const element = state.appData[index];
                        if (element.moduleId === value) {
                            activationItem.updateActivationItem({ ...element });
                            break;
                        }
                    }
                }
            ...
            case 'updateAppData':
                ...
                if (!deepEqual(state.appData, value)) {
                    ...
                    const asynAcactivationItem = (value as AppDataListTypes).find(
                        (item) =>
                            item.moduleId === state.activationItem.moduleId
                    );
                    if (asynAcactivationItem?.moduleId) {
                        activationItem.updateActivationItem(
                            asynAcactivationItem
                        );
                    }
                }
                break;
