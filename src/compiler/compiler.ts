...
export const backgroundGroup = (
  backgroundGroupObj: BackgroundGroupTypesOfStyleItems,
): resultType => {
  ...
  backgroundList?.forEach((background, index) => {
    ...
    if (background.imageUrl) {
      ...
      // 获取当前浏览器类型
      if (prefix === 'webkit') {
        backgroundResultArray.push(
          `${gradientLise[1]} ${backgroundItemStyle.join(' ')}`,
        );
      } else if (prefix === 'moz') {
        backgroundResultArray.push(
          `${gradientLise[0]} ${backgroundItemStyle.join(' ')}`,
        );
      } else {
        backgroundResultArray.push(
          `${gradientLise[2]} ${backgroundItemStyle.join(' ')}`,
        );
      }
    }
  });
