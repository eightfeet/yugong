 ...
 */
 ...
 const Output: OutputModules<Props> = ({ pageData }) => {
   ...
   const globalLoading = useCallback(
     (statusArg: ArgumentsItem) => {
       // 'show' | 'hide' | 'reset' 显示，隐藏，重置
       const status = getArgumentsItem(statusArg) as 'show' | 'hide' | 'reset';
       if (status === 'show') loading.show();
       if (status === 'hide') loading.hide();
       if (status === 'reset') loading.reset();
     },
     [],
   )
