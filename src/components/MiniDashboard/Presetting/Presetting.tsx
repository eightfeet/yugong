...
import React, { useCallback, useMemo, lazy, useState } from 'react';
...
import {
    AppDataLayoutItemTypes,
    ArgumentsNumber,
    ArgumentsString,
} from '~/types/appData';
import { EventsTypeItem, ExposeFunctions } from '~/types/modules';
 ...
 * 作为预设模块只操作组件自己的内置方法
 * */
const Presetting: React.FC<Props> = ({ custom, exposeFunctions }) => {
    /**获取当前激活组件信息*/
    ...
    const updateActDataToAll = useCallback(
        (actData: AppDataLayoutItemTypes) => {
            ...
            dispatch.appData.updateAppData(
                produce(
                    operateData,
                    undefined,
                    createDesc(
                        '修改组件',
                        `${actData.moduleName || actData.moduleId} 预设`
                    )
                )
            );
    ...
    const updateRunningDataToActivationItem = useCallback(
        (copyRunningData: ExposeFunctions[]) => {
            ...
            if (!copyModuleData.events?.mount) {
                ...
                mount.forEach((mountItem) => {
                    copyRunningData.forEach((runningDataItem) => {
                        if (
                            mountItem.name ===
                            `${moduleId}/${runningDataItem.name}`
                        ) {
                            if (runningDataItem.arguments)
                                mountItem.arguments = runningDataItem.arguments;
                        }
                    });
                });
                // 遍历ready数据
